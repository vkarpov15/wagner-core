...
var wagnerFactory = function(name) {
  ...
  var wagner = {
    ...
    invokeAsync: function(func, locals) {
      ...
      step({}, serviceCache, alreadyExecuted, orderedTasks, function(error) {
        if (error) {
          if (hasErrorParameter) {
            var args = [error];
            for (var i = 0; i < paramNames.length; ++i) {
...
function step(state, serviceCache, alreadyExecuted, tasks, callback) {
    ...
    if (ready) {
      ...
      if (paramNames.hasCallback) {
        args.push(function(error, value) {
          ...
          step(state, serviceCache, alreadyExecuted, tasks, callback);
        });
